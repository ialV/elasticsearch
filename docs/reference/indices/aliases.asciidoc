...
在一个单一的原子操作中执行一个或多个 <<aliases,别名>> 操作

[source,console]
----
POST _aliases  // 向 _aliases 发送 POST 请求
{
  "actions": [  // 定义要执行的操作列表
    {
      "add": {  // 添加操作
        "index": "my-data-stream",  // 指定要添加的数据流或索引名称
        "alias": "my-alias"  // 指定要添加的别名
      }
    }
  ]
}
...
* 如果启用了 {es} 的 {security-features}，您必须具有以下 <<privileges-list-indices,`索引权限`>>：

** 要使用 `add` 或 `remove` 操作，您必须对别名及其数据流或索引具有 `manage` 索引权限。

** 要使用 `remove_index` 操作，您必须对索引具有 `manage` 索引权限。
